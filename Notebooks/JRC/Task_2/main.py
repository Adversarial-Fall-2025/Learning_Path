...
X = samples_array.astype(np.float32) / 255.0
y = y_labels
...
idx = 5  # change to any index in your test set
img_flat = X_test[idx].reshape(1, -1)  # flattened image (256*256*3)

# Transform with PCA and scaler
img_pca = pca.transform(img_flat)
img_scaled = scaler.transform(img_pca)

# Convert to tensor and move to device
img_tensor = torch.tensor(img_scaled, dtype=torch.float32).to(device)
...
plt.title(f"Predicted class: {pred_class} | Probabilities: {probs}")
plt.axis('off')
plt.show()
